...
### lmer version with just prompt_format
summary(lmer(data = df_summ,
           mean_accuracy ~ prompt_format + (1|model_shorthand)))




# Plot with error bars and raw points
set.seed(1)
plot_data <- df_summ %>%
...
ggplot(plot_data, aes(x = prompt_format, y = mean_acc)) +
  geom_point(size = 4, color = "steelblue") +
  geom_errorbar(aes(ymin = mean_acc - se, ymax = mean_acc + se), 
                width = 0.2, linewidth = 1, color = "steelblue") +
  geom_jitter(data = df_summ, 
              aes(x = prompt_format, y = mean_accuracy),
              width = 0.15, alpha = 0.3, size = 2, color = "gray40") +
  theme_minimal() +
  labs(x = "Prompt Format",
       y = "Mean Accuracy") +
  theme(text = element_text(size = 15))
